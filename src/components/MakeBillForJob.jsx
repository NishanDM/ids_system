...
const handleAddStockItemToBill = (stockItem) => {
 ...
 // 1️⃣ Populate customer fields
    ...
    const deviceLine = `${jobData.deviceType || "-"} | ${jobData.model || "-"}${faultsText ? ` | ${faultsText}` : ""}`;
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-[95%] md:w-3/4 lg:w-2/3 max-h-[90vh] overflow-auto p-6 m-4">
        ...
        />
                      <button
                        ...
                        className="px-3 py-2 bg-blue-600 text-white rounded flex items-center gap-1 hover:bg-blue-700 cursor-pointer"
                      >
                        <Search size={16} />
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-none">
    <div className="bg-white p-6 rounded shadow-lg w-80 text-center">
      ...
      <div className="flex justify-around gap-4">
        <button onClick={confirmClose} className="px-4 py-2 bg-red-500 text-white rounded">Yes</button>
        <button onClick={cancelClose} className="px-4 py-2 bg-gray-200 rounded">No</button>
      </div>
    </div>
  </div>
)}
